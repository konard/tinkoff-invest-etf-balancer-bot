...
function buildPrompt(content: string, id: string, symbol: string): string {
  return [
    `Ты опытный финансовый аналитик. Проанализируй новость по фонду ${symbol}.`,
    'Верни строго JSON без пояснений. Поля:',
    '{',
    '  "id": string,                                   // идентификатор новости',
    '  "symbol": string,                               // тикер фонда',
    '  "title": string,                                // заголовок',
    '  "date": string,                                 // дата из новости как есть',
    '  "category": string,                             // тип: ребалансировка|дивиденды|погашение паев|прочее',
    '  "summary": string,                              // краткое содержание в 1-3 предложениях',
    '  "bullets": string[],                            // ключевые пункты (3-8)',
    '  "trades": [                                     // если есть таблица сделок',
    '    { "ticker": string, "name": string, "side": "Покупка"|"Продажа", "qty": string, "amount": string, "weightFrom": string|null, "weightTo": string|null }',
    '  ],',
    '  "additionalFields": { [name: string]: string },  // например: Дата погашения паев, Погашено паев, На сумму, Всего паев, Цена пая',
    '  "numbers": {                                     // нормализованные числа, если можно извлечь',
    '    "redeemedShares": number|null,                 // шт, без суффиксов',
    '    "redeemedAmountRub": number|null,              // ₽',
    '    "totalShares": number|null,                    // шт',
    '    "navPriceRub": number|null                     // ₽',
    '  }',
    '}',
    '',
    'Текст новости ниже между <news>...</news>. Сохраняй исходные форматы строк в summary/fields, но числа в numbers делай числовыми.',
